...
detectar_ataque_ip<-function(datos){
  ...
  return (probabilidad_ataque)

}

datos <- read.csv("~/tarea2/data/data.csv")  # Se procede a leer la data suministrada

probabilidad_ataque_final <- 0

probabilidad_ataque_final <- detectar_ataque_ip(datos)

print(paste0("La probabilidad de que el log de datos suministrados tenga un ataque de ip por fuerza bruta es: ",probabilidad_ataque_final))
